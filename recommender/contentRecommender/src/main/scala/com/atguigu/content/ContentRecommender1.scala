...
object ContentRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConf = new SparkConf().setMaster(config("spark.cores")).setAppName("ContentMovieRecs")
    ...
    //核心部分，用TF-IDF从内容信息中提取特征向量
    //1.创建一个分词器，默认按空格分词
    val tokenizer = new Tokenizer().setInputCol("genres").setOutputCol("words")

    //2.用分词器对原始数据做转换，生成新的一列words
    val wordsData = tokenizer.transform(movieTagDF)

    //3.引入HashingTF工具，得到idf词频模型
    ...
    //5.训练idf模型，得到每个词的逆文档频率
    val idfModel = idf.fit(featurizeData)

    //6.用idf模型对原始数据进行处理，得到文档中每个词的tf-idf,作为新的特征向量
